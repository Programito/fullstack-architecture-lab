...
exports.MESAFLOW_DEMO_RESTAURANT_NAME = exports.MESAFLOW_DEMO_ORGANIZATION_NAME = void 0;
exports.seedMesaFlowDemo = seedMesaFlowDemo;
const DEMO_ORGANIZATION_NAME = 'MesaFlow Demo';
const DEMO_RESTAURANT_NAME = 'MesaFlow Centro';
async function seedMesaFlowDemo(prisma) {
    ...
    for (const item of menuItems) {
        ...
        if (!restaurantProductId)
            throw new Error(`Missing restaurant product for "${item.productName}".`);
    ...
    for (const { productName, groupId, sortOrder } of [
        ...
        if (!restaurantProductId)
            throw new Error(`Missing restaurant product for modifier group assignment: "${productName}".`);
    ...
    if (!comboProductId)
        throw new Error('Missing combo product.');
    ...
    const comboSlotOptions = [
        ...
        if (!restaurantProductId)
            throw new Error(`Missing combo slot option product "${opt.productName}".`);
        return {
    ...
    const platterComponentNameI18n = {
    ...
    for (const platter of [
        ...
        if (!platterProductId)
            throw new Error(`Missing platter product "${platter.productName}".`);
...
function demoRestaurantProducts() {
    return [
...
exports.MESAFLOW_DEMO_ORGANIZATION_NAME = DEMO_ORGANIZATION_NAME;
exports.MESAFLOW_DEMO_RESTAURANT_NAME = DEMO_RESTAURANT_NAME;
//# sourceMappingURL=mesaflow-demo.seed.js.map