...
describe('ProductEditorPage', () => {
  ...
  it('pre-fills the per-language name fields (CA/EN/ES) from nameI18n in edit mode', async () => {
    getProduct.mockReturnValue(
      of({ ...MOCK_PRODUCT, nameI18n: { ca: 'Hamburguesa artesana', en: 'Craft burger', es: 'Hamburguesa artesanal' } }),
    );
    await renderPage('rp-1');
  ...
  it('sends nameI18n.es when the Spanish display name is filled in and omits empty languages', async () => {
    ...
    expect(createProduct).toHaveBeenCalledWith(
      expect.objectContaining({ name: 'Bocadillo', nameI18n: { es: 'Bocata mixto' } }),
    );
  });
