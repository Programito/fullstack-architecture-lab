...
export class ListRestaurantsUseCase {
  ...
  async execute(restaurantIds: string[], organizationIds: string[]): Promise<Result<RestaurantSummary[], never>> {
    return ok(await this.restaurants.listRestaurants(restaurantIds, organizationIds));
  }
}
