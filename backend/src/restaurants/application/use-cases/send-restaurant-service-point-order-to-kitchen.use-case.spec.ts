...
import { SendRestaurantServicePointOrderToKitchenUseCase } from './send-restaurant-service-point-order-to-kitchen.use-case';
...
function makeServicePoint(): ServicePointDetailView {
  return {
    table: { id: 'table-1', tableNumber: 1, name: 'Mesa 1', capacity: 2, status: 'waiting_kitchen', occupiedAt: null, serviceStartedAt: null },
    floorElement: null,
...
describe('SendRestaurantServicePointOrderToKitchenUseCase', () => {
  it('calls sendPendingLinesToKitchen and updates table status when persistent order has pending lines', async () => {
    ...
    expect(orders.sendPendingLinesToKitchen).toHaveBeenCalledWith('restaurant-1', 'table-1');
    expect(restaurants.setServicePointStatus).toHaveBeenCalledWith('restaurant-1', 'table-1', 'waiting_kitchen');
  ...
  it('returns invalid_service_action when persistent order has no pending lines', async () => {
    ...
    const orders = makeOrderRepository();
    vi.mocked(restaurants.findFloorsByRestaurantId).mockResolvedValue(makeFloors());
    vi.mocked(orders.findActiveByTable).mockResolvedValue(makeOrderWithNoEligibleLines());
    const useCase = new SendRestaurantServicePointOrderToKitchenUseCase(restaurants, orders);
  ...
  it('falls back to demo path when no persistent order exists for the table', async () => {
    ...
    vi.mocked(restaurants.findServicePointOrderByRestaurantId).mockResolvedValue({
      ...
      lines: [{ id: 'l-1', productName: 'Burger', quantity: 1, unitPriceCents: 1200, subtotalCents: 1200, status: 'pending', course: 'mains', kitchenNote: null }],
    });
    vi.mocked(restaurants.sendServicePointOrderToKitchen).mockResolvedValue(demoServicePoint);
    const useCase = new SendRestaurantServicePointOrderToKitchenUseCase(restaurants, orders);

    const result = await useCase.execute('restaurant-1', 'table-1');

    expect(result).toEqual(ok(demoServicePoint));
    expect(restaurants.sendServicePointOrderToKitchen).toHaveBeenCalledWith('restaurant-1', 'table-1');
    expect(orders.sendPendingLinesToKitchen).not.toHaveBeenCalled();
  });
