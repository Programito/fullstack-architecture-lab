...
import { UpdateRestaurantOrderLineUseCase } from './update-restaurant-order-line.use-case';
...
describe('UpdateRestaurantOrderLineUseCase', () => {
  it('delegates the update command to the repository and returns the updated order', async () => {
    const repository = makeRepository();
    const updatedOrder = makeOrder();
    vi.mocked(repository.updatePendingLine).mockResolvedValue(updatedOrder);
    const useCase = new UpdateRestaurantOrderLineUseCase(repository);
    ...
    expect(repository.updatePendingLine).toHaveBeenCalledWith({
      restaurantId: 'restaurant-1',
      orderId: 'order-1',
      lineId: 'line-1',
      quantity: 3,
  ...
  it('returns invalid_order_configuration when quantity is zero or negative', async () => {
    const repository = makeRepository();
    const useCase = new UpdateRestaurantOrderLineUseCase(repository);
    ...
    expect(repository.updatePendingLine).not.toHaveBeenCalled();
  });

  it('returns invalid_order_state when the line is not pending', async () => {
    const repository = makeRepository();
    vi.mocked(repository.updatePendingLine).mockRejectedValue(
      new ApplicationErrorException(applicationError('invalid_order_state', 'Line cannot be edited in its current state.')),
    );
    const useCase = new UpdateRestaurantOrderLineUseCase(repository);
  ...
  it('rethrows unexpected errors', async () => {
    const repository = makeRepository();
    vi.mocked(repository.updatePendingLine).mockRejectedValue(new Error('DB connection lost'));
    const useCase = new UpdateRestaurantOrderLineUseCase(repository);

    await expect(
      useCase.execute({ restaurantId: 'restaurant-1', orderId: 'order-1', lineId: 'line-1', quantity: 1 }),
